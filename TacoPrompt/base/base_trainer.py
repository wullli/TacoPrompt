...
class BaseTrainer:
    ...
    def test(self):
        ...
        for i, mtr in enumerate(self.metrics):
            self.logger.info('    {:15s}: {:.3f}'.format('test_leaf_' + mtr.__name__, leaf_values[i]))
            wandb.run.summary[f'test_leaf_{mtr.__name__}'] = leaf_values[i]
        for i, mtr in enumerate(self.metrics):
            self.logger.info('    {:15s}: {:.3f}'.format('test_nonleaf_' + mtr.__name__, nonleaf_values[i]))
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            for key, value in result.items():
                ...
                elif key == 'leaf_val_metrics':
                    log.update({'val_leaf' + mtr.__name__: value[i] for i, mtr in enumerate(self.metrics)})
                elif key == 'nonleaf_val_metrics':
                    log.update({'val_nonleaf' + mtr.__name__: value[i] for i, mtr in enumerate(self.metrics)})
