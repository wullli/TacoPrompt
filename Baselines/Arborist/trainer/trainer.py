    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            if self.is_infonce_training:
                ...
                if self.config['loss'].startswith("DistMarginLoss"):
                    # print(label)
                    loss = self.loss.loss(prediction, label, nf)
                    # loss = self.loss.loss(prediction, label, sorted(list(self.data_loader.dataset.tain_node_ids)))
                else:
    ...
    def _test(self, mode, gpu=True):
        ...
        with torch.no_grad():
            ...
            for i, query in enumerate(vocab):
                # print(node2pos[query][0][1])
                # print(dataset.pseudo_leaf_node)
