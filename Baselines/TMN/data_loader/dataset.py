...
class MAGDataset(object):
    ...
    def _load_dataset_raw(self, dir_path):
        ...
        # print(len(self.g_full.nodes()))
        # print(len(self.g_full.edges()[0]))
        # exit()
        ...
        if self.existing_partition:
            ...
            if self.partition_pattern == 'leaf':
                ...
                print(len(root_node))
                # print([tx_id2node_id[node.tx_id] for node in taxonomy.nodes()])
...
class RawDataset(Dataset):
    def __init__(self, graph_dataset, mode="train", sampling_mode=1, negative_size=32, max_pos_size=100,
        ...
        print(f"name:{graph_dataset.name}")
        print(f"train_set:{len(graph_dataset.train_node_ids)}")
        print(f"val_set:{len(graph_dataset.validation_node_ids)}")
        print(f"test_set:{len(graph_dataset.test_node_ids)}")
        ...
        # with open('train_node_ids_tmn.pkl','wb') as f:
