...
class UnifiedDataLoader(DataLoader):
    def __init__(self, mode, data_path, sampling_mode=1, batch_size=10, negative_size=20, max_pos_size=100,
                 expand_factor=50, shuffle=True, num_workers=0, cache_refresh_time=64, normalize_embed=False,
                 test_topk=-1, test=0, num_sib=5):
        self.mode = mode
    ...
    def collate_fn(self, samples):
        if 'g' in self.mode and 'p' in self.mode:
            us, vs, ss, graphs_u, graphs_v, paths_u, paths_v, lens, queries, labels = map(list, zip(*chain(*samples)))
            ...
            for i in range(len(ss)):
                if len(ss[i]) < self.num_sib:
                    sib_s[i] = ss[i] + [self.dataset.pseudo_leaf_node] * (self.num_sib - len(ss[i]))
                else:
                    sib_s[i] = ss[i]
            if 's' in self.mode:
