...
class TrainerT(TrainerS):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            self.optimizer.zero_grad()
            scores, scores_p, scores_c, scores_e, scores_s = self.model(nf, u, v, sib, bgu, bgv, bpu, bpv, lens, sib_s)
            label = label.to(self.device)
            loss_p = self.loss(scores_p, label[:, 1])
            loss_c = self.loss(scores_c, label[:, 2])
            loss_e = self.loss(scores_e, label[:, 0])
            loss_s = self.loss(scores_s, label[:, 0])
            loss = self.loss(scores, label[:, 0]) + self.l1 * loss_p + self.l2 * loss_c + self.l3 * loss_e + self.l4 * loss_s
            loss.backward()
            self.optimizer.step()
                ...
                self.logger.debug(
                    'Train Epoch: {} [{}/{} ({:.0f}%)] Loss: {:.6f} ELoss: {:.6f} PLoss: {:.6f} CLoss: {:.6f} SLoss: {:.6f}'
                        .format(epoch, batch_idx * self.data_loader.batch_size, self.data_loader.n_samples,
                                100.0 * batch_idx / len(self.data_loader),
                                loss.item(), loss_e.item(), loss_p.item(), loss_c.item(), loss_s.item()))
